...
def list_scsi_luns(
    ...
    """
    List SCSI Luns on the ESXi datacenter/cluster/host.
    ...
    state
        Disk state, [attached, detached] (optional)
    ...
    .. code-block:: bash
        ...
        salt '*' vmware_esxi.list_scsi_luns datacenter_name=dc1 cluster_name=cl1 host_name=host1
    """
    ...
    for host in hosts:
        try:
            ...
            for scsi_lun in storage_system.storageDeviceInfo.scsiLun:
                if lun_name and lun_name in scsi_lun.devicePath:
                    ret[host.name].append({
                        "state": state_map[scsi_lun.operationalState[0]],
                        "name": scsi_lun.displayName,
                        "path": scsi_lun.devicePath,
                        "device": scsi_lun.deviceName,
                        "uuid": scsi_lun.uuid,
                        "descriptor": [(d.id, d.quality) for d in scsi_lun.descriptor if d.quality == 'highQuality'],
                        "local": scsi_lun.localDisk,
                        "location": scsi_lun.physicalLocation,
                    })
                elif state and state_map[state] == scsi_lun.operationalState[0]:
                    ret[host.name].append({
                        "state": state_map[scsi_lun.operationalState[0]],
                        "name": scsi_lun.displayName,
                        "path": scsi_lun.devicePath,
                ...
                elif lun_name == None and state == None:
                    ret[host.name].append({
                        "state": state_map[scsi_lun.operationalState[0]],
                        "name": scsi_lun.displayName,
                        "path": scsi_lun.devicePath,
...
def get_host_disks(
    ...
    profile=None):
    """
    Return a list of hosts and the LUNs that are assigned to them.
    ...
    ids = {}
    for host in hosts:
        if host.name not in ids:
            ids[host.name] = []
        for datastore in host.datastore:
            for extent in datastore.info.vmfs.extent:
                if (disk_name and disk_name == datastore.info.vmfs.name) or \
                    not disk_name:
                    ids[host.name].append({
                        "scsi_address": extent.diskName, 
                        "name": datastore.info.vmfs.name,
                        "uuid": datastore.info.vmfs.uuid,
                        "local": datastore.info.vmfs.local
                    })
    return ids
...
def rescan_storage(
    ...
    log.debug("Running vmware_esxi.rescan_storage")
    ret = {}
    service_instance = service_instance or utils_connect.get_service_instance(
        config=__opts__, profile=profile
    )
    ...
    for host in hosts:
        try:
            storage_system = utils_common.get_storage_system(service_instance, host, host.name)
            res = storage_system.RefreshStorageSystem()
            if not res:
                ret[host.name] = True
            else:
                ret[host.name] = res
...
def mount_storage(lun_canonical_name,
    datacenter_name=None,
    cluster_name=None,
    host_name=None,
    service_instance=None,
    ...
    log.debug("Running vmware_esxi.mount_storage")
    ret = {}
    service_instance = service_instance or utils_connect.get_service_instance(
        config=__opts__, profile=profile
    )
    ...
    for host in hosts:
        try:
            log.debug(f"mounting {lun_canonical_name} on {host.name}")
            storage_system = utils_common.get_storage_system(service_instance, host, host.name)
            storage_system.AttachScsiLun(lun_canonical_name)
            ret[host.name] = True
            #ret[host.name] = f"mounting {lun_canonical_name} on {host.name}"
        except Exception as err:
            ret[host.name] = f"Error: {err}"
...
def unmount_storage(lun_canonical_name,
    datacenter_name=None,
    cluster_name=None,
    host_name=None,
    service_instance=None,
    ...
    log.debug("Running vmware_esxi.unmount_storage")
    ret = {}
    service_instance = service_instance or utils_connect.get_service_instance(
        config=__opts__, profile=profile
    )
    ...
    for host in hosts:
        try:
            log.debug(f"unmounting {lun_canonical_name} on {host.name}")
            storage_system = utils_common.get_storage_system(service_instance, host, host.name)
            #storage_system.DetachScsiLun(lun_canonical_name)
            ret[host.name] = str(dir(storage_system))
        except Exception as err:
            ret[host.name] = f"Error: {err}"
